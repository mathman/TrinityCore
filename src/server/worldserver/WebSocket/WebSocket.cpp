...
void WebSocketThread(const std::string& host, const std::string& port)
{
    try
    {
        ...
        WebSocket web(io_service, endpoint_iterator);
...
void WebSocket::run()
{
    ...
    std::array<int8, READ_BLOCK_SIZE> buffer;
    ...
    message.append(buffer.data(), len);

    handler_buffer(message);
}

void WebSocket::handler_buffer(WebByteBuffer buffer)
{
    _readBuffer = buffer;
    uint8 opcode;
    _readBuffer >> opcode;
    _readBuffer.setOpcode(opcode);
}

void WebSocket::ReadHandler()
{
    switch (_readBuffer.getOpcode())
    {
        case WEB_DEFAULT:
            break;
        default:
